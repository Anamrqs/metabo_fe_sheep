...
sed -i 's/PREDICTIONS/leucine/g' template_V3.R
...
sed -i 's/RANGE_Y/11/g' template_V3.R
sed -i 's/CORRECTION/corr/g' template_V3.R

# Modif script name
mv template_V3.R model10_Leucine_Buckets_corr_V3.R

# Run Rscript, output a logfile -- prediction ingestion --
Rscript --vanilla --verbose model10_Leucine_Buckets_corr_V3.R > slurm-${SLURM_JOB_ID}.Rout 2>&1 
...
mv model10_Leucine_Buckets_corr_V3.R /work/$USER/Scripts_V3/
mv $SCRATCHDIR ~/work/
