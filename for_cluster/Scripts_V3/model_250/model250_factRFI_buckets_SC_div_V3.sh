...
sed -i 's/DATASET/df_SC_rfi_V3/g' template_V3.R
...
sed -i 's/RANGE_X/c(18:length(df_prediction))/g' template_V3.R
sed -i 's/RANGE_Y/6/g' template_V3.R
sed -i 's/CORRECTION/SC_div/g' template_V3.R

# Modif script name
mv template_V3.R model250_factRFI_Buckets_SC_V3.R

# Run Rscript, output a logfile -- prediction ingestion --
Rscript --vanilla --verbose model250_factRFI_Buckets_SC_V3.R > slurm-${SLURM_JOB_ID}.Rout 2>&1 
...
mv model250_factRFI_Buckets_SC_V3.R /work/$USER/Scripts_V3/
mv $SCRATCHDIR ~/work/
