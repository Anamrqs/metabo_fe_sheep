...
sed -i 's/PREDICTIONS/fcr/g' template_V3.R
...
sed -i 's/RANGE_Y/9/g' template_V3.R
sed -i 's/CORRECTION/SC/g' template_V3.R

# Modif script name
mv template_V3.R model250_FCR_All_SC_V3.R

# Run Rscript, output a logfile -- prediction ingestion --
Rscript --vanilla --verbose model250_FCR_All_SC_V3.R > slurm-${SLURM_JOB_ID}.Rout 2>&1 
...
mv model250_FCR_All_SC_V3.R /work/$USER/Scripts_V3/
mv $SCRATCHDIR ~/work/
