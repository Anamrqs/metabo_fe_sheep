...
sed -i 's/LIST_KEEPX/c(seq(1,5,1),10,15,22)/g' template_V3.R
...
# Modif script name
mv template_V3.R model250_Ingestion_Zootech_SC_V3.R

# Run Rscript, output a logfile -- prediction ingestion --
Rscript --vanilla --verbose model250_Ingestion_Zootech_SC_V3.R > slurm-${SLURM_JOB_ID}.Rout 2>&1 
...
mv model250_Ingestion_Zootech_SC_V3.R /work/$USER/Scripts_V3/
mv $SCRATCHDIR ~/work/
