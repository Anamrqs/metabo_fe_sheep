...
library(tidyverse) #dplyr::select
library(rsample)
library(mixOmics) #purrr::map
library(tictoc)
...
#formule Quentin pour selectionner les hyperparametres
...
inner = function (objet, model, rangeX, rangeY) { #ex model : spls or splsda
  X.train = training(objet)[rangeX] #ex rangeX : c(8:ncol(NMR_subset))
  X.test = testing(objet)[rangeX] 
  
  Y.train = training(objet)[[rangeY]] #ex : rangeY = 5 (indice de la lignee)
